...
@Author: Ulysses
@LastEditTime: 2019-10-12 08:52:45
'''
...
def get_shortest_path(graph, v0):
    paths = dijkstra_shortest_paths(graph, v0)
    paths_1 = [([], None)] * len(paths)
    pre = None
    for i in range(len(paths)):
        pre = i
        paths_1[i] = ([pre], paths[i][1])
        while True:
            pre = paths[pre][0]
            paths_1[i][0].insert(0, pre)
            if pre == v0:
                break
    return paths_1
